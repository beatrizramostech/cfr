...
const FormularioInicial = ({ dados, onNext }) => {
  ...
  useEffect(() => {
    ...
    if (tipoSolicitacao === 'proprio' && usuario) {
      ...
      form.setValue(
        'interessado.cnhOrgaoEmissor',
        usuario.cnhOrgaoEmissor || ''
      );
      form.setValue('interessado.cnhuf', usuario.cnhuf || '');
      form.setValue(
        'interessado.cnhValidade',
        formatDate(usuario.cnhValidade) || ''
      );
          ...
          <fieldset>
            ...
            <div className="row">
              <label className="input-group">
                <div
                  className={
                    tipoSolicitacao === 'terceiro' ? 'tooltip-container' : ''
                  }
                >
                  Nome *
                  <input
                    className={
                      tipoSolicitacao === 'proprio' ? 'disabled-input' : ''
                    }
                    {...register('interessado.nome')}
                    readOnly={tipoSolicitacao === 'proprio'}
                  />{' '}
                  <span className="tooltip-text">Campo obrigatório</span>
                </div>
              </label>
